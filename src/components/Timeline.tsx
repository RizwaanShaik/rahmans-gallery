...
export default function Timeline() {
  ...
  const getCategoryTextColor = (category: Category) => {
    ...
    const darkColors = {
      'early-life': 'dark:text-blue-200',
      'education': 'dark:text-green-200',
      'career': 'dark:text-purple-200',
      'exhibition': 'dark:text-amber-200',
      'award': 'dark:text-red-200',
      'international': 'dark:text-indigo-200'
    };
    return `${lightColors[category] || 'text-gray-800'} ${darkColors[category] || 'dark:text-gray-200'}`;
  };
  ...
  const getCategoryBgColor = (category: Category) => {
    ...
    const darkColors = {
      'early-life': 'dark:bg-blue-900/30',
      'education': 'dark:bg-green-900/30',
      'career': 'dark:bg-purple-900/30',
      'exhibition': 'dark:bg-amber-900/30',
      'award': 'dark:bg-red-900/30',
      'international': 'dark:bg-indigo-900/30'
    };
    return `${lightColors[category] || 'bg-gray-50'} ${darkColors[category] || 'dark:bg-gray-800/30'}`;
  };
            ...
            <div key={index} className={`relative flex items-center ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
              ...
              <div className={`w-5/12 ${index % 2 === 0 ? 'pr-8 text-right' : 'pl-8 text-left'}`}>
                <div className={`p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100 dark:border-gray-700 ${getCategoryBgColor(event.category)}`}>
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium mb-2 border dark:border-gray-600 ${getCategoryTextColor(event.category)} bg-white dark:bg-gray-800`}>
              ...
              {/* Center Point */}
              <div className="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 rounded-full border-4 border-white dark:border-gray-900 shadow-sm flex items-center justify-center">
            ...
            <div key={index} className="relative pl-12">
              ...
              <div className={`p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100 dark:border-gray-700 ${getCategoryBgColor(event.category)}`}>
                <div className="flex justify-between items-center mb-2">
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium border dark:border-gray-600 ${getCategoryTextColor(event.category)} bg-white dark:bg-gray-800`}>
                    {getCategoryLabel(event.category)}
                  </span>
                  <span className="bg-gray-800 text-white px-2 py-1 rounded font-bold text-sm">
                    {event.year}
